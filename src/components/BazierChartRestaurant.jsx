...
const BazierChartRestaurant = () => {
    ...
    const fetchSales = async (period) => {
        ...
        let url = `${BaseurlRestuarant}restaurant-orders-by-restaurantId/${restuarantId}?type=${period}&selectedValue=${selectedOption}`
        // Add restaurantId only if selectedRestaurant has a value
        if (selectedSupplier) {
            console.log('selectedRestaurant', selectedSupplier);
            url += `&supplierId=${selectedSupplier}`;
        }
        const headers = {
            "x-access-token": restaurantToken,
        }
        try {
    ...
    const fetchSuppliers = async () => {
        ...
        const response = await fetch(`${BaseurlRestuarant}get-supplier-by-restaurant-id/${restuarantId}`, { headers });
        const json = await response.json();
        setSupplier(json);
    }
    ...
    useEffect(() => {
        ...
        fetchSuppliers(); // Fetch restaurants data for supplierId
    }, [view, selectedOption, selectedSupplier]);
...
export default BazierChartRestaurant;
