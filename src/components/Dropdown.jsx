...
export default function Dropdown({ title, hint, marginTop, width, selectedOption, onValueChange }) {
  const [showOptions, setShowOptions] = useState(false);
  const options = ['Canada']; // Example options array

  const handleOptionSelect = (option) => {
    onValueChange(option); // Pass selected option back to parent
  ...
  return (
    <View style={[styles.container, { marginTop: marginTop, width: width }]}>
      ...
      >
        ...
        <Image 
          resizeMode='contain' 
          style={{ position: 'absolute', height: 20, width: 20, right: 20 }} 
          source={require('../../assets/images/Cart/down.png')} 
        />
      </TouchableOpacity>
